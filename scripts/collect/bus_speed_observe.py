...
try:
    ...
    from app.modules.travel_ops.mobility_engine.timeutil import day_type_of
    _HOL = set(json.loads((REPO / "final_project_cs" / "app" / "modules" / "travel_ops" / "mobility_engine" / "rules" / "holidays_2026_2027.json")
...
if day_type_of is None:
    raise SystemExit("요일축을 못 읽었다 — final_project_cs/app/modules/travel_ops/mobility_engine/rules/holidays_2026_2027.json 과 PYTHONPATH 를 확인한다")
...
result = {
    ...
    "method": "같은 차량(plainNo)의 연속 관측을 run 으로 묶고, run 안의 모든 간격을 거리 0 까지 포함해 "
              ...
              "요일축(weekday/holiday)은 app.modules.travel_ops.mobility_engine.timeutil.day_type_of 와 같은 자로 나눈다.",
    "grade": "추정", "routes": {},
}
