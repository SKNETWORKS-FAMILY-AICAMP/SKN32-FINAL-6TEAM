...
배울 수 있으면 시간표 46만 행이 필요 없고, 못 배우면 "판정은 코드"가 숫자로 서는 것이다.
  ...
  A · 현행  : 시간표 조회 + 규칙 코드 (final_project_cs/app/modules/travel_ops/mobility_engine/verify_time.py)
...
from app.modules.travel_ops.mobility_engine.timeutil import day_type_of, to_service_min          # noqa: E402
...
def main():
    ...
    ap.add_argument("--holidays", default=str(REPO / "final_project_cs" / "app" / "modules" / "travel_ops" / "mobility_engine" / "rules" / "holidays_2026_2027.json"))
