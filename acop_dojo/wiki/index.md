...
`[2026-09-16 신설]` **처음부터 쌓기(`build`)** 는 반대 방향이다 — 빈 폴더에서 베이스먼트를 직접 만든다. 이 모드만 대상이 `final_project_sample` 이다(코어가 `acop_basement/` 한 패키지에 모여 있어 "코어만 비우고 다시 쌓기" 가 성립한다). 순서는 import 관계 실측에서 나왔고(7층·순환 0), 단계별 판정 테스트도 단계까지의 모듈만 넣은 폴더를 만들어 세었다. 자세한 것은 [guide.md](guide.md).
