...
★`[2026-09-23 해결]` **이 화면에는 로그인이 없었다.** 승인·위임 버튼이 서버 안에서 scope 키를 스스로 만들어
써서 `/ui` 에 닿기만 하면 누구나 누를 수 있었고, 승인자는 전부 `ui-operator` 였다. 이제 `/ui/*` 전체가
**로그인한 운영자**만 보고, 승인·바깥함 해소는 `action:approve`, 위임 변경은 `delegation:write` 가 있어야 한다.
승인자·위임 행위자는 **그 운영자 id** 로 남는다. 결정 [D-CS-007](../decisions/D-CS-007-ui-operator-login.md) ·
