...
| 위임을 주고 거두는 화면·API | `[2026-09-22 해결]` REST `/v1/delegations/*` + 운영 화면 `/ui/delegations`. 맡기기는 **무엇이 열리는지 보여 주는 확인 단계**를 거치고, 거둘 것이 없으면 `409` 로 사유가 화면에 뜬다. 이력은 `delegation_events`(021)에 덧붙는다. ★**남은 구멍 — `/ui/*` 에는 로그인이 없다**(승인 화면과 같다) |
