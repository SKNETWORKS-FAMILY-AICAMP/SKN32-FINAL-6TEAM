...
| 오늘 실제로 열었는가 | ~~**모른다.** '미확인'으로 표시한다~~ `[정정 2026-09-10]` 아래 결정 15 정정 참조 — 개점 확인 값도 대상이다. |
| 지금 대기가 얼마인가 | ~~**모른다.**~~ 모델이 채워 넣지 못하게 한다. `[정정 2026-09-10]` 아래 결정 15 정정 참조 — 대기 값도 대상이다. |
...
`[실측]` 같은 파일의 `_unknown()`은 "값이 없는 게 아니라 모르는 상태"를 escalate로 보낸다. `[정정 2026-09-10]` 위 「영업시간 데이터는 개점 확인이 아니다」 절의 결정 15 정정 참조 — 코드 관찰이다.
