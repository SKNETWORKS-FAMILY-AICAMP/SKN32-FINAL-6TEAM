...
```
Controller(＋Case 상태기계) → 흐름을 결정
Message Broker             → 배달
```

`[실측 2026-09-10 정정]` 첫 줄은 「Top-Level LangGraph → 흐름을 결정」이었다. `requirements.txt` 에 `langgraph==1.2.8` 이 있지만 **코드 어디서도 import 하지 않는다**(cs·sample·composer 0곳, 동적 import 도 0곳).
