...
★2026-09-10 커머스판(`tests/unit/voc/test_feedback.py`)에서 옮겼다. 검사 논리는
  그대로다 — 바뀐 것은 라벨 어휘뿐이다. 옛 파일이 잡아낸 사각지대 주석도 함께
  옮긴다. 그 교훈은 도메인과 무관하기 때문이다.
"""
...
def test_classifier_returns_all_four_fields_from_injected_llm():
    result = classify(
        "비 온다는데 내일 골프 취소되나요?",
        lambda _: {"sentiment": "negative", "intent": "confirm_request",
                   "issue_code": "activity_weather_risk", "severity": "high"},
    )
    assert result.sentiment == "negative"
    assert result.intent == "confirm_request"
    assert result.issue_code == "activity_weather_risk"
                    ...
                    for t, customer in zip((tenant, other_tenant), customers):
                        cur.execute(
                            """INSERT INTO customer_cases (tenant_id,customer_id,status,subject,intent,issue_code,sentiment,created_at)
                               VALUES (%s,%s,'running','voc','incident_report','activity_weather_risk','negative',%s)""",
