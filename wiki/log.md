...
| 문서 | 무엇이 바뀌었나 |
|---|---|
| [research/dietary-data-sources.md](research/dietary-data-sources.md) | **신규(분할).** 식이 제약 쪽 실측 + 관광공사 공식 PDF |
| [research/accessibility-data-sources.md](research/accessibility-data-sources.md) | **신규(분할).** 무장애 API 실측 + 지하철 승강기 후보 |
| [product/traveler-profile.md](product/traveler-profile.md) | 식이 제약을 다루는 방법을 결론 쪽으로 옮겼다 |

`[실측 2026-09-22]` **무장애 여행 정보 활용신청이 반영됐다** — 같은 공통 키로 200 이 온다. 전국 9,638 · 서울 547(관광지 162 · 음식점 105). 서울 30건 표본 채움률은 `route` 83% · `exit` 83% · `restroom` 53% · `elevator` 33%, 반면 `stroller` 7% · `babysparechair` 0%. ★**값이 자연어 문장이라 boolean 으로 파싱하지 않는다.** 커버리지가 관광지 42%(162/382) · 음식점 11%(105/968)라 **탈락 조건으로는 못 쓰고 원문 표시 + 순위 가산으로 쓴다.**

`[실측 2026-09-22]` **할랄은 관광공사가 무료로 공개하고 있었다** — 영문 사이트의 `Muslim-friendly_restaurants.pdf`(52.1 MB, 118쪽)에 **약 370곳(좌표쌍 373, 중복 제거 358)**, 서울 약 86곳. 상호·주소·전화·영업시간·가격대·좌석·주차·**위경도**까지 들어 있다. 레드테이블이 1,100만원에 파는 것과 **같은 원천**이다. 단 형식이 PDF 라 한 번 뽑아 정리해야 하고 갱신이 수동이다.

`[갱신]` `.env.apikeys` 와 `.env.apikeys.example` 에 무장애 서비스 주석을 넣었다 — **새 변수는 만들지 않았다**(선언되지 않은 이름을 적으면 앱이 기동을 못 한다). 공통 키로 열린다.

---

## 2026-09-22 — 여행자 프로필: 물어보기 전에 데이터를 먼저 불러 봤다

설문 초안 열두 항목을 **데이터 확인 없이** 판정했다가 둘이 뒤집혔다. 「알러지는 식당 대안의 탈락 조건」과 「유아·노약자는 이동 제약으로 받자」 둘 다 **대응 데이터가 없어서 못 쓴다.** v11 결정 15(값을 모르는 상황을 만들지 않는다)에 정면으로 걸리는 항목이었다.

| 문서 | 무엇이 바뀌었나 |
|---|---|
| [product/traveler-profile.md](product/traveler-profile.md) | **신규.** 받는 값 넷(언어·인원·최대 도보 시간·식사 제약 유무)과 뺀 항목의 이유. 식이 제약·무장애 실측 |
| [product/index.md](product/index.md) | 읽기 순서와 문서 표에 추가 |

`[실측 2026-09-22]` 확인한 값 — TourAPI 「할랄」 전국 1건(행사), 「무슬림」 0건. 음식점 상세 16개 필드에 식이 항목 없음. 서울 등록 음식점 968건. 카카오 로컬은 관련도 정렬이라 「터키」 1위가 써브웨이라 필터로 못 씀. 무장애 전용 서비스(`KorWithService2`)는 403. `chkbabycarriage` 서울 관광지 40건 중 「있음」 **0건**, `kidsfacility` 서울 음식점 30건 전부 `0`.

구글 `servesVegetarianFood`는 존재하나 **키가 없어 호출하지 못했다** — Enterprise + Atmosphere, $25/1,000건, 월 1,000건 무료. 카드 등록된 Cloud Billing 계정이 필요하다. 한국 식당 채움률은 `[미확보]`.

★**같은 날 분리** — 문서가 300줄을 넘어 **근거를 [research/dietary-data-sources.md](research/dietary-data-sources.md) · [research/accessibility-data-sources.md](research/accessibility-data-sources.md)로 옮겼다.** `product/traveler-profile.md`는 결론(받는 것 넷·안 받는 것과 이유)만 남겼다.

★**무장애 403 의 원인을 가렸다** — 본문이 `SERVICE_KEY_IS_NOT_REGISTERED_ERROR`다. 경로가 틀린 게 아니라 활용신청이 안 된 것이고(1판은 폐기), `data.go.kr` `/data/15101897` 에서 **개발단계 자동승인·일일 1,000건·무료**로 신청할 수 있다. 지하철 승강기는 후보 셋을 찾았으나 전부 「역에 엘리베이터가 있다」까지라 **환승 경로의 계단 여부는 못 준다** — 경로 엔진을 정해야 붙일 자리가 생긴다.

★**레드테이블 데이터는 미리보기로 실물을 봤다** — 할랄인증구분명·인증기관명·기도실여부 컬럼이 있고 라벨이 제대로다(공식 인증 1·프렌들리 12·자가인증 3·포크프리 3). 인증기관이 한국관광공사라 **무료 경로가 있을 수 있다.** 앞서 적은 「6,084건」은 조회수였다 — 건수는 화면에 없다.

★**같은 날 보강** — 「무슬림」 한 낱말로만 찾고 「데이터가 없다」고 적은 것을 정정했다. 「할랄」로 다시 찾아 **한국문화정보원_전국 세계 음식점 데이터**(9,502건, 무료)를 받아 전수 집계했다. `할랄음식 여부`·`채식메뉴 보유여부`·`휠체어 보유여부` 열이 실제로 있지만 **할랄 Y 전국 22건(0.2%)**이고 **무료주차 Y가 9,502건 전부(100%)**다 — 플래그를 믿을 수 없다. 서울 할랄 Y 11곳에 수제맥주집과 우동집이 들어 있고 기준일은 2022-11-30이다. 전국으로 넓혀도 결론은 같다.
...
| [index.md](index.md) | 「여행 판올림」 절 신설 — 낡음 실측(198문서 중 104개 53%가 커머스 낱말), 영역별 승계 여부. DoD 29 → 22, business 원가가 쇼핑몰 것이라는 표시. 미결정 표를 v11 §0-3 로 교체 |
...
| [cs/wiki/index.md](../final_project_cs/wiki/index.md) | 코어는 승계·Team 페이지 다섯은 쇼핑몰이라는 표시. v11 §5-A 의 코어 변경 넷과 Trip·`trigger_source` |
...
| 지표 | 기준 | 실측 | 판정 |
|---|---|---|---|
| 애매 비율 | 15% 이하 | **25%** (11/44 — `[미확보 2026-09-10]` 표본이 45건인데 분모가 44다. 뺀 한 건의 이유가 안 적혔다) | **미달** |
...
`[2026-09-10]` **날짜 없이 적힌 옛 이관 예정 목록이다** — 지금 할 일 목록으로 읽지 않는다. `A-COP_구현계획서_v8.md` 는 이제 `program/plan/` 에 없고 압축 보존본 `program/plan/.archive/` 로 갔다(루트 CLAUDE.md). 각 행이 끝났는지는 [governance/migration.md](governance/migration.md) 가 정본이다. 그때 쓴 문장 — 아래는 아직 `program/plan/`에 있고 이 wiki로 옮겨야 한다.
