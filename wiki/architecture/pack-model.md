...
**그래도 이 문서를 지우지 않는다.** 낱말이 사라졌을 뿐 **기제는 v10 이 그대로 전제하고 있다** — v11 §0-2 가 "코어는 승계하고 Team 만 갈아 끼운다"로 판올림 전체를 정의하는데, 그게 성립하려면 아래 「교체가 성립하는 조건」 넷이 필요하다. 여기가 그 조건을 적어 두는 자리다.
...
```text
   ...
   Case · Controller · Registry · Port · 승인 경계 · 감사 · 평가
        ...
        ┌───────────────┴───────────────┐
   여행 도메인 Pack                 (다음 도메인)
   Activity        MVP               갈아 끼우는 자리
   Dining          MVP (4주차)
   Mobility        MVP (5주차)
   Booking Handoff MVP 다음
   Lodging/Flight  등록만
```
...
`[실측 2026-09-09]` **사용자가 방향을 지시했다** — 콘솔 UI 에서 관리 가능하게 한다. 갈래 넷과 판정은 [../decisions/D-015-implementation-catalog.md](../decisions/D-015-implementation-catalog.md) 로 옮겼다.
...
| **착수 (MVP)** | **Activity · Dining · Mobility 셋** — Dining 4주차 · Mobility 5주차 (v11 §9-B). Booking Handoff 는 MVP 다음 |
...
| 순서 | Team | 근거 |
|---|---|---|
| **MVP** | **Activity** | 취소·변경 규정이 문서로 존재해 판정 규칙을 바로 쓴다. 예약금이 걸려 실패 비용이 크다 |
| **MVP** (4주차) | **Dining** | |
| **MVP** (5주차) | **Mobility** | 선제 조정 루프와 함께 |
| MVP 다음 | Booking Handoff | 우리 일정을 고쳐도 업체 예약을 못 바꾸면 고객이 직접 처리한다. `[결정 2026-09-10]` MVP 에서 뺐다 — 09-08 승격을 뒤집음 |
| 등록만 | Lodging / Flight | 잠긴 예약으로만 취급한다 |

**6명 팀 전체가 이 구성으로 고정된다는 뜻은 아니다.**

실제 업체 예약 실행은 Mock 으로 남긴다 — 실결제는 구현 단계 4다(v11 §4-C).
