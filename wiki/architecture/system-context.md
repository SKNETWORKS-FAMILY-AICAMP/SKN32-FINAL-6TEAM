...
| 행위자 | 무엇을 하나 | 경로 |
|---|---|---|
| 고객 (인바운드 여행자) | 일정을 제출하고 사건을 신고한다 | **여행계획서 링크**(여행별 토큰, 로그인 없음) → REST |
...
### ★ [2026-09-10 오후] 고객 접점이 둘이다

`[결정]` v11 §1·§9-C — **자체 앱은 만들지 않지만 고객이 직접 보는 주소가 하나 생겼다.**

| # | 접점 | 무엇 | 왜 |
|---|---|---|---|
| ① | **개인 에이전트 API** | 일정을 넘기고 통지를 받는다 | 우리는 API 로만 만난다 |
| ② | **여행계획서 링크** | 최신 일정 · 무엇이 왜 바뀌었나 · **재요청 창구**. 여행별 토큰, 로그인도 계정도 없다 | **상태의 정본.** 알림이 유실돼도 여기서 맞는 것을 본다 |

★**②가 「통지가 닿았나」를 사양에서 없앤다**(DoD-26 계열). 알림은 신호이고 이 화면이 사실이다.

★**자체 앱이 아니다.** 고객이 일정을 직접 편집하지 않는다 — 재요청은 **API 로 들어온 것과 같은 경로**(요청 → 검증 → 적용)를 지난다.
