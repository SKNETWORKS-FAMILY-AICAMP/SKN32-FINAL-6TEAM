...
| ~~지원 언어 범위~~ | **닫힘 (2026-09-10, v11 §0-4).** ~~우리는 **한국어 하나**만 내고 **고객 언어 번역은 고객 에이전트 몫**이다. ★**번역 품질을 보증하지 못하므로 「다국어 지원」이라고 말하지 않는다**~~ `[정정 2026-09-10]` 세 번 바뀌었다 — 한국어 하나(결정 2) → 한국어+영어(결정 11) → 보낼 때 고객 언어로 생성(결정 14): 개인 에이전트 API는 한국어로 내고 에이전트가 옮기므로 「다국어 지원」이라 말하지 않으며, 링크·알림은 언어 목록 없이 우리가 고객이 고른 언어로 생성하고 그 경로의 번역·생성 품질 보증은 새 과제로 둔다(v11 §0-4 결정 14는 품질 보증을 정하지 않는다). |
...
## ★ [2026-09-10] Codex 교차 검수가 잡은 것 — 계획서 쪽 둘

`[실측]` Codex 독립 검수 2026-09-10. **내(문서 세션) 작업과 계획서를 따로 대조해 찾은 것이고, 아래 둘은 계획서 담당 몫이다.**

| # | 무엇이 어긋나나 | 어느 쪽이 맞나 |
|---|---|---|
| ~~1·2~~ | ~~v11 요약표 둘이 본문과 어긋났다(§0-2A DoD 24 · §0-4 「여덟」)~~ | **둘 다 닫힘 (2026-09-10)** — 계획 세션이 DoD 26 · 「팀장 결정 열둘」로 고쳤다 |
| **4** | **datasets 원본 REPORT·STATUS 22건의 오류 — 데이터 담당 몫** | codex 가 `wiki/_migration/` 스테이징 사본에서 찾았다. **사본은 고치지 않는다** — 오류의 원본은 `datasets/*/REPORT.md·STATUS.md` 이고 원본 문서는 패치로 고친다. 목록은 전수검사 산출물 |
| **3** | **★평가 러너가 정답을 모델 입력에 넣는다 — 코드 담당** | `eval/runners/common.py` 가 골든 행 전체를 프롬프트에 넣고(세 군), B 의 검색과 Proposed 의 라우팅·capability 를 정답으로 고른다. 쇼핑몰 시절 분류 100% 는 능력이 아니었다. **여행 평가 전에 고쳐야 한다** → [../evaluation/protocol.md](../evaluation/protocol.md) |

★**둘 다 같은 기제다** — 본문을 고치고 요약을 안 고쳤다. v11 §0-2A 와 §0-4 는 **다른 절을 요약하는 표**라서 본문이 움직이면 조용히 어긋난다.

`[미확보]` **복제된 표의 동등성을 검사하는 방법이 없다.** v11 §12 ↔ [dod.md](dod.md) ↔ [milestones/final-2026-10-26.md](milestones/final-2026-10-26.md) 가 독립 사본이고, Team manifest ↔ 각 Team 페이지도 그렇다. 한쪽만 고치면 다시 갈린다.

### 검사기가 못 잡는 것 — Codex 가 일곱 가지를 셌다

`[실측]` 그중 **하나는 고쳤다.** 나머지는 남는다.

| 허점 | 상태 |
|---|---|
| 코드 경로 검사가 **git 추적 여부를 안 본다** — 미커밋 구현도 「존재」로 통과 | **고쳤다** (2026-09-10). 저장소별로 `git ls-files` 를 세어 따로 집계한다 |
| `domain: travel` 문서 **안의** 현재형 커머스 단정은 안 본다 | `[미확보]` |
| `domain_note` 가 있으면 통과한다 — **그 설명이 낡았는지는 안 본다** | `[미확보]` |
| **「N파일」 같은 수치 단정**을 검사하지 않는다 | `[미확보]` 실제로 `travel_ops 7파일` 이라 적었는데 9였다 |
| 파일 **안의 심벌·값**(`accepted_case_types`·intent 목록·함수명·줄번호)을 안 본다 | `[미확보]` 실제로 manifest 축이 틀려 있었다 |
| **복제 표 동등성** 검사가 없다 | `[미확보]` 위 |
| **기준선의 의미 결정**(언어·멱등 대상·라우팅 축)은 `domain:` 과 코드경로 검사 양쪽을 빠져나간다 | `[미확보]` |

★**마지막 줄이 이번 사고의 정체다.** 「우리가 내는 언어」가 영어에서 한국어로 바뀐 것은 **파일이 없어진 것도 도메인이 바뀐 것도 아니라** 사양이 바뀐 것이다. 두 검사 다 사양을 모른다.
...
| 항목 | 어디 |
|---|---|
| ~~**[코드 담당 인계 2026-09-23] ① 확정 시나리오의 이동 표기를 새 계약에 맞춘다**~~ ★`[완료 2026-09-23]` 버스 2016·2224·7022(서울시 TOPIS 정류소별 경유 노선으로 확인) + `도로:아차산로`, 환승역 두 노선 각각, `도보:` 제거 — 재생 사건 대상은 그대로. [기록](../../final_project_cs/wiki/records/evidence/2026-09-23_경로_uses_표기_검사.md) — 일정 등록 몸통의 `uses`(이동 수단이 지나는 역·노선·도로를 적는 칸, 감시가 이 문자열로 운행·통제 사건을 대조한다) 표기가 2026-09-23 계약이 됐다. `final_project_cs/app/modules/travel_ops/scenarios/seoul_day_taiwan_friends.json` 에서 어긋나는 곳: ⓐ `버스:성수동` 2곳(`konkuk_bus`·`ttukseom_bus`) — 동네 이름이 아니라 **버스 노선번호**로(`버스:<번호>`). 번호는 지어내지 말고 서울 버스 노선 자료로 확인한다 ⓑ `도보:서울역`(`dinner_to_mart`) — `도보:` 는 안 쓴다. 큰길을 안 지나면 `[]` ⓒ 갈아탄 역 누락 — `seongsu_to_palace` 는 을지로3가(`2호선:을지로3가`·`3호선:을지로3가`), `palace_to_dinner.subway_transfer` 는 충무로(`3호선:충무로`·`4호선:충무로`) ⓓ `bus_sejong` 에 `버스:<번호>` 를 `도로:세종대로` 와 함께. ★재생 타임라인의 사건 대상(`2호선:성수`·`도로:세종대로`, 140·145행)은 계약과 맞아 **안 바꾼다** — 바꾸면 시연 사건이 안 걸린다. 끝나면 확정 시나리오 하루 시험이 그대로 통과해야 한다 | [계약](../../final_project_cs/wiki/external/rest-endpoints.md) 「`routes{<키>}.options[].uses`」 · [리포트](../../final_project_cs/wiki/records/reports/2026-09-23_1817_경로_uses_표기_계약.md) |
| ~~**[코드 담당 인계 2026-09-23] ② 일정 등록 때 `uses` 형식을 검사한다**~~ ★`[완료 2026-09-23]` **거절로 정했다** — `422 invalid_route_uses` + 틀린 값 전부. 틀린 표기 셋이 201 로 받아지는 것을 먼저 봤다. [기록](../../final_project_cs/wiki/records/evidence/2026-09-23_경로_uses_표기_검사.md) — 지금 `POST /v1/trips`·`POST /v1/trips/plan`(`register:true`) 은 `uses` 에 무엇이 와도 받는다. `잠실역`·`02호선:잠실`·`버스:성수동` 처럼 계약에 없는 모양이 들어오면 **오류 없이 저장되고, 나중에 그 구간 사건을 조용히 못 잡는다.** 권장: 계약 형식(`<노선명>:<역명>` · `버스:<노선번호>` · `도로:<도로명>`)에 안 맞으면 `422` 로 거절하고 어느 값이 왜 틀렸는지 적는다 — 받아 두고 경고만 하면 보낸 쪽이 모른다. 거절 대신 경고로 갈지는 코드 담당이 정하되 **조용히 받는 것은 안 된다.** 시험은 틀린 표기 셋을 넣어 **빨간 것을 먼저 본다** | [계약](../../final_project_cs/wiki/external/rest-endpoints.md) 「`[미반영]`」 |
| **[코드 담당 인계 2026-09-23] ③ 이동 데이터 모듈이 들어오면 입구에서 표기를 바꾼다** — `uses` 는 **바깥 계약**이라 공식 표기(`2호선`)를 쓴다. 이동 담당이 작업 중인 이동 판정 엔진(미병합 — `team/role-mobility`)은 내부에서 노선명을 두 자리(`02호선`)로 정규화한다. 그 엔진이 들어올 때 **엔진 입구의 어댑터가 바꾼다** — 외부 계약도 엔진 데이터도 고치지 않는다. 누가 어댑터를 만들지는 **이동 담당과 합의할 일**이고, 사건 입력 규격(`current_state.replan` — 이동 담당이 제안만 해 둔 상태)도 같이 정한다. 지금 할 일은 없다 — 병합 시점에 잊지 않게 둔다 | [계약](../../final_project_cs/wiki/external/rest-endpoints.md) · [리포트](../../final_project_cs/wiki/records/reports/2026-09-23_1817_경로_uses_표기_계약.md) §4 |
...
| ~~**★라우팅 축 — 적용만 남았다**~~ **작업 트리에 적용됐다 (2026-09-10).** `travel_ops/feedback.py` 의 `INTENTS` 다섯이 슬러그이고 `ISSUE_CODES` 가 여행 17개, `controller.py:73,176` 이 `case_type_of(issue_code)` 와 `intent` 를 둘 다 넘긴다. ★**`[실측 git]` 아직 커밋 전이다** — 되돌려지면 다시 열린다. 아래는 그때의 서술이다. `[실측 2026-09-09]` 안 A(두 축)를 **시뮬레이션으로 검증**했다 — `case_type`=객체 종류, `intent`=요청 종류로 넣으면 **6팀 전부 라우팅되고**, 요청 종류가 capability 네임스페이스와 안 맞아도 **안 좁힐 뿐 실패하지 않는다**. `case_type` 은 `issue_code` 접두에서 뽑으므로 **계약이 안 바뀐다**. 제안한 `ISSUE_CODES` 17개가 **17/17 라우팅 성공**. 고칠 자리는 셋 — 분류 어휘 2줄 · 컨트롤러 2줄 · 가드 테스트. ★가드 테스트는 **일부러 깨뜨려 빨간 것을 본 뒤** 고쳐야 한다(지금 것이 통과하면서 못 잡고 있었다) | [../../program/plan/A-COP_여행_분류어휘와_라우팅_명세.md](../../program/plan/A-COP_여행_분류어휘와_라우팅_명세.md) |
...
| ~~**★여행 Case 가 지금 라우팅되지 않는다**~~ **작업 트리에서 풀렸다 (2026-09-10).** 위 항목과 같은 구현이다. 남은 것은 **커밋**이다. 아래는 그때의 서술이다. `[실측 2026-09-09]` 여행 Team 6개가 등록·해석까지 다 됐는데(`config/project.yaml`, 6/6), **분류기 어휘가 아직 쇼핑몰**이다. `INTENTS` 5개·`ISSUE_CODES` 13개가 전부 커머스라 여행 `case_type` 6개(`activity`·`booking`·`mobility`·`dining`·`lodging`·`flight`)를 **하나도 만들어 내지 못한다** — 실행해 확인했다(`ClassificationFailed`). **2026-08-17 사고와 같은 모양**이고, 재발 방지로 만든 `test_feedback_intent_alignment.py` 가 **등록조차 안 된 팀 하나만 봐서 계속 통과한다** | [../../program/plan/A-COP_여행전환_현황_2026-09-09.md](../../program/plan/A-COP_여행전환_현황_2026-09-09.md) |
| ~~**★한 Case 에 같은 종류 제안이 둘이면 두 번째가 조용히 사라진다 — 결정 필요**~~ `[정정 2026-09-10]` 아래 「business_subject」 규칙 항목의 정정 참조 — 결정은 끝났고 Core 구현이 남았으며, 뒤는 당시 재현·대안 기록이다. `[실측·검토 2026-09-10]` 서버가 Team 의 키를 버리고 **항상 `case_id`** 로 다시 만들고(`controller.py:363`), 충돌하면 `ON CONFLICT DO UPDATE ... RETURNING` 이 **기존 행의 id 를 돌려주며 두 번째 인자를 버린다**. 살아 있는 DB 에서 재현했다(롤백) — 예약 두 건이 한 행이 되고 `bk-002` 가 사라진다. ★**지금은 잠재 결함이다**(적대적 검토가 잡음) — 여행 Team 은 전부 제안 하나씩 돌려준다. 다만 `TeamResult` 는 여럿을 허용하고, **승인 화면·실행 직전 재검증이 저장된 행을 읽으므로 못 되살린다.** ★`case_id` 선택엔 근거가 있다(`2026-08-14_S-IDEMKEY_리포트`: 결제 식별자가 계약 필드로 없던 모델). **여행이 그 전제를 깬다.** 정할 것 — ①「한 Case 에 같은 종류 하나」를 사양으로 삼고 저장 전 거부 ② 서버가 대상 객체 id 를 키에 넣기 | [../../program/plan/A-COP_여행전환_현황_2026-09-09.md](../../program/plan/A-COP_여행전환_현황_2026-09-09.md) §4 |
| ~~**`business_subject` 3단 폴백이 여행 공용 기반으로 옮겨 왔다**~~ **닫힘 — 전제가 틀렸다 (2026-09-10).** Team 쪽 값은 저장되지 않는다. 원래 문구: `[실측 2026-09-09]` `travel_ops/_base.py:169` 이 `booking_id → trip_id → case_id` 폴백을 쓴다. 전에는 한 팀에만 있던 것이 **이제 공용 기반이라 여행 팀 전부가 물려받는다.** `booking_id` 가 `None` 이면 조용히 `case_id` 로 떨어지고 한 Case 안의 두 예약이 같은 idempotency 키를 갖는다. **Trip 하나에 예약이 여럿인 여행이 커머스보다 위험하다** | [../../program/plan/A-COP_여행전환_현황_2026-09-09.md](../../program/plan/A-COP_여행전환_현황_2026-09-09.md) |
| ~~**여행 MVP 설계 결정 5건 — 초안 나왔고 결정이 남았다**~~ **대부분 닫힘 (2026-09-10).** v11 §0-4 가 **팀장 결정 여덟**으로 닫았다 — 도시 서울 · 언어 한국어 하나 · 알림은 무료 채널 하나로 고객에게 직접 통지 · 지원한다고 넣은 장소는 전부 되게 한다 · 무료·무예약 활동도 Activity 가 처리 · 감시 소스는 범용 API 부터 업체로 넓힌다 · 손익은 「얼마 들고 얼마 받으면 남나」까지 · 강사 「에이전트」 해석은 더 확인하지 않는다(판정 기준 변경). **남은 셋은 v11 §0-3** — 통지 수신 확인 · 기준 상품 2도시와 MVP 1도시의 충돌 · 가격 수용. 아래는 그때의 서술이다. `[2026-09-09]` 계획 생성·지속관리 루프·알림 경로·데이터 구분·화면을 코드 실측 위에 정리했다. **결정이 필요한 것 넷** — ① 정규화 다섯 칸에 `constraints`(동행 조건)를 넣을지 ② 감시 소스 설정을 Team manifest 에 둘지 `config/` 에 둘지 ③ 알림 수신 주체가 사람인지 고객 에이전트인지 ④ 기상 API 를 실제로 붙일지 Mock 으로 할지(시연 재현성). ★**`business_subject` 규칙과 정규화 다섯 칸이 먼저다** — 둘 다 계약이라 나중에 바꾸면 그 위가 전부 따라 바뀐다 | [../../program/plan/A-COP_여행MVP_설계질문_5건.md](../../program/plan/A-COP_여행MVP_설계질문_5건.md) |
| ~~**[코드 담당 인계] 도메인 무관 가드가 여행 어휘를 모른다**~~ **닫힘 (2026-09-10).** `DOMAIN_WORDS` 에 `booking`·`itinerary`·`lodging`·`supplier_booking`·`traveller` 가 들어갔고 **옛 어휘는 지우지 않았다** — 커머스 벌에 「2026-09-10 에 코드에서 나갔다」 주석이 붙었다. 아래는 그때의 서술이다. `[실측 2026-09-09]` `final_project_cs/tests/architecture/test_basement_is_domain_free.py` 의 `DOMAIN_WORDS` 에 구독·결제·커머스 어휘만 있다. `trip`·`itinerary`·`booking`·`reservation`·`activity` 가 `app/core/` 에 들어가도 통과한다 — 여행 Team 구현 시작이 가장 새기 쉬운 시점이다. **옛 어휘는 지우지 않고 더한다**(되돌아갔을 때도 잡히게). sample 쪽 같은 파일도 같다 | [cs/wiki/domain-swap.md](../../final_project_cs/wiki/domain-swap.md) 2026-09-09 절 |
| ~~**`business_subject` 에 무엇을 넣는지 규칙이 없다 — 계획서 담당 몫**~~ `[정정 2026-09-10]` Team의 폴백 값은 최종 멱등 키에 쓰이지 않고 Core가 `app/application/controller.py:371-374`에서 `business_subject=str(case["case_id"])`로 다시 계산한다(2026-09-10 실측). `booking_id` 유무와 무관하게 같은 Case·같은 종류의 작업이 서로 다른 객체를 바꾸면 충돌할 수 있다. Team은 대상 id를 제안하고 서버는 인자에서 대상을 꺼내 실재·소유를 확인해야 한다 — activity·dining은 예약이 있으면 `booking_id`, 없으면 `item_id`, mobility는 `item_id`이며 특정하지 못하면 폴백하지 않고 거부한다(v11 §4-E). 사양 결정은 닫혔지만 Core의 §4-E 구현은 남았다. **닫힘 (2026-09-10).** v11 이 **§4-E 를 신설해** 정했다 — 서버가 인자에서 꺼내 실재·소유를 확인한 대상 객체 id 를 쓰고, 특정 못 하면 폴백하지 않고 거부한다. 접두별 인자 표도 있다(`activity`·`dining` 은 예약 있으면 `booking_id`, 없으면 `item_id`). **DoD-24 로 검사 항목이 됐다.** ~~★코드는 아직 3단 폴백이다(`travel_ops/_base.py:169`) — 구현이 남았다.~~ `[정정 2026-09-10]` 이 항목 첫 정정 참조 — 결함 자리는 Core다. 아래는 그때의 서술이다. `[실측 2026-09-09]` 코어는 이미 도메인 중립이다(`app/core`·`app/application` 에 `order_id`·`booking_id`·`shipment_id` **0회**). 도메인 객체는 `idempotency_key(...)` 의 `business_subject` 한 칸으로 들어가는데 **팀마다 다른 걸 넣는다** — `case_id` 3곳 · `shipment_id` 1곳 · `order_id→customer_id→case_id` 3단 폴백 1곳. ★키가 `f(tenant, request_id, action_type, subject)` 이고 `request_id` 는 Case 당 하나라, **`subject=case_id` 면 한 Case 안의 두 객체가 같은 키를 갖는다**(실측 확인). 쇼핑몰은 Case 하나=주문 하나라 안 드러났지만 **여행은 Trip 하나에 예약이 여럿**이고 사건 하나가 여러 예약을 바꾼다. 정할 것은 이름이 아니라 규칙이며, **v11 §6 이 키 산식을 안 적고 있다** | [../../program/plan/A-COP_여행Team모듈_구성안.md](../../program/plan/A-COP_여행Team모듈_구성안.md) |
