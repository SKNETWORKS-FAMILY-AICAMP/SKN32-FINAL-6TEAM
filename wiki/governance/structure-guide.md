...
**6단계다**(Level 0~5). `[정정 2026-09-10]` 5단계로 적혀 있었다 — 아래 그림이 0부터 센다. **Level 0~3** 은 다음 단계로 가는 **길만 알려주고 지식은 담지 않는다.** 지식은 **Level 4(개념 문서)**, 구현은 **Level 5(소스 코드)** 에 있다 — 「모든 단계가 길만 안내한다」로 읽으면 그 둘과 모순이다.
...
| 결제 소유 경계 | `wiki/decisions/` | cs와 검증 쇼핑몰 둘 다 당사자 |
| 건당 원가 | `wiki/business/` | 특정 커밋에 안 묶인다 |
| Shared State 불변식 | `<저장소>/wiki/runtime/` | 테스트가 강제한다 |
| **Core/Team 경계 자체** | `wiki/architecture/` | **어느 구현에도 안 묶인다** |
| DoD 29항목 | `wiki/delivery/` | 여러 저장소에 걸친다 |
