...
★`[미확보 2026-09-10]` **아래 제15조가 현행 원문인지 다시 대조하지 못했다**(법제처 페이지를 이 수정에서 읽지 못했다). codex 전수검사가 두 곳을 지적했다 — 제4호 끝(현행은 「…계약을 체결하는 과정에서 정보주체의 요청에 따른 조치를 이행하기 위하여 필요한 경우」라 한다)과 제5호 앞머리(「정보주체 또는 그 법정대리인이 의사표시를 할 수 없는 상태에 있거나 주소불명 등으로 사전 동의를 받을 수 없는 경우로서」는 2023년 개정에서 빠진 조건이다). 2023년 개정 내용과도 맞는 지적이다 `[추정]`. **대조 전까지 「원문」이 아니라 옛 조회본으로 읽는다.**
...
| 대금 환급 | **3영업일 이내** | 기산점이 유형별로 다르다. 반품은 **반환받은 날** (법 §18① — `[미확보 2026-09-10]` codex 전수검사는 3영업일 환급이 **②** 라고 했다. 이 수정에서 원문을 다시 읽지 못해 확정하지 않았다) |
