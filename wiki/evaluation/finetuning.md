...
## ★ [2026-09-06] 지위 — 채택 안 함은 제외가 아니다

`[실측]` 사용자 정리. 파인튜닝은 **제외한 것이 아니라 계속 시도하는 트랙**이다. 지금까지의 시도(stage3 v1~v9, 2026-09-02까지)는 채택 기준 미달이라 채택하지 않았을 뿐이고, 진척이 있으면 적용한다. 임베딩·리랭커 파인튜닝도 후보다. 결과가 나쁘면 나쁘다고 그대로 보고한다.

"결과는 부정적"·"채택 불가"는 **그 시점 그 모델에 대한 판정**이지 트랙의 종료가 아니다. 09-01 stage3-v8에서 보인 "효과"(mismatch echo 100%→50%)는 09-02 v9에서 n=1→4 표본 잡음이자 무효 지표 위의 값으로 확인됐고([dod28-rerun](dod28-rerun.md)), **그 처방의 확대는 거기서 멈췄다** — 파인튜닝 자체를 멈춘 게 아니다. 다음 시도는 다른 처방(데이터 종류·대상 모델·임베딩/리랭커)으로 한다.
