...
type: decision
title: 결제 소유 경계
description: 결제 실행은 검증 쇼핑몰이 소유하고 triPilot은 구성을 읽어 대조만 한다
status: draft
...
| 무엇 | 커머스에서 | **여행에서** |
|---|---|---|
| 결정 | **결제를 우리가 실행하지 않는다** | 그대로. v11 §4-C — **실결제는 구현 단계 4** |
| 돈이 움직이는 자리 | 쇼핑몰이 결제·환불을 실행한다 | **업체 예약 변경이 그 자리다.** 승인 없이 건드리지 않고 **변경 링크로 넘긴다** |
| 우리가 하는 것 | 금액을 **대조**한다 | 그대로. 환급액이 **예약 금액**을 넘는지 본다 |
| 시연 한정 예외 | 없었다 | **협약사 Mock 자동 실행**(`tier == 'simulated'`). 아키텍처 테스트로 막는다(DoD-14·15) |

★**같은 위험이 여행에서 더 크다.** 아래 「환불 계산식을 고쳐야 한다」는 **금액 칸이 하나뿐이라 할인·쿠폰이 들어갈 자리가 없다**는 문제였다. 여행에서는 **위약금율이 업체마다 다르고 규정 원문을 표본으로만 갖는다** — 계산 근거를 우리가 덜 안다. → [`action-proposal.md`](../../final_project_cs/wiki/actions/action-proposal.md)

★**그래서 이 문서를 레거시로 옮기지 않는다.** 판정의 **이유**가 지금도 유효하다 — 돈이 움직이는 실행을 우리가 갖지 않고, 금액은 대조만 한다.

`[실측]` **`[미확보]` 하나가 남는다.** 쇼핑몰 쪽 「조치안」(환불 계산식 전환)은 **대상이 사라져 실행될 일이 없다.** 여행 쪽 대응(위약금 규정을 어디까지 신뢰하나)은 아직 정해지지 않았다.

---

## 맥락

Team 이름에 "Payment"가 들어가 있고 `payment.status` capability가 있어서, 결제를 triPilot이 어디까지 다루는지 불명확했다. 검증 쇼핑몰과 연계하기 전에 경계를 확정해야 했다.

포인트·캐시·쿠폰을 넣을지도 같이 물어봤다.

## 결정

> **결제 실행은 검증 쇼핑몰이 소유한다. triPilot은 결제 구성을 읽어 설명하고 대조만 한다.**
...
| 안 | 채택 | 이유 |
|---|---|---|
| triPilot이 결제를 소유 | ❌ | 파는 물건의 성격이 바뀐다. PCI-DSS 부담이 고객사가 아니라 우리에게 온다 |
| 잔액만 triPilot이 관리 | ❌ | 도입 기업엔 이미 결제 시스템이 있다. 이중 장부가 된다 |
| **읽기만. 실행은 쇼핑몰** | ✅ | 현재 구조와 일치. 규제 부담 없음 |
| 결제를 아예 안 봄 | ❌ | 환불 금액을 설명할 수 없다. 골든셋 42%가 금액 관련이다 |

### 근거 4가지

**1. 우리가 파는 물건이 그게 아니다.** triPilot은 고객 응대를 구성하는 플랫폼이다. 결제를 가지면 커머스 시스템이 되고 "결제를 우리 것으로 바꾸라"고 요구하는 제품이 된다.
...
### [2026-09-01 보강] 절반은 지금 데이터로도 고칠 수 있다

`[실측]` `order_items`에 **`unit_cents`(품목별 단가)가 이미 있다.**

```sql
order_items (order_item_id, tenant_id, order_id, sku, name, quantity, unit_cents)
```

즉 두 가정 중 하나는 지금 당장 버릴 수 있다.

| 문제 | 해결 가능한가 |
|---|---|
| **모든 품목 값이 같다** | **가능.** `order_items.unit_cents`로 품목별 배분 |
| **할인이 없다** | **불가.** 실결제액 칸이 없다 → 쇼핑몰이 줘야 함 |

**조치 1을 두 단계로 나눌 수 있다는 뜻이다.**

| 단계 | 무엇 | 의존 |
|---|---|---|
| 1-A | 균등 분할을 품목별 배분으로 교체 | **없음. 지금 가능** |
| 1-B | 쇼핑몰 계산값 대조로 전환 | 검증 쇼핑몰 협의 |

**1-A를 먼저 하면 쇼핑몰 일정을 안 기다려도 된다.** 다품목 주문의 오차가 즉시 사라진다.

## 못 하게 되는 것

- 결제 관련 질문에 triPilot 단독으로 답할 수 없다. 쇼핑몰 응답이 필요하다
...
| # | 조치 | 소유 | 성격 | 의존 |
|---|---|---|---|---|
| **1-A** | **균등 분할 → `order_items.unit_cents` 품목별 배분** | cs | 버그 수정 | **없음** |
| 1-B | 환불 금액을 **쇼핑몰 계산값 대조** 구조로 변경 | cs | 계약 변경 | 5번 |
| 2 | `read.payment` 도구 추가 | cs | 신규 | 5번 |
| 3 | `orders`에서 결제 상태 분리 판단 | 스키마 | 스키마 | — |
| 4 | `mock_payment_publisher.py` 파일명 정리 | cs | 정리 | — |
| 5 | 검증 쇼핑몰에 필요 응답 필드 전달 | 검증 쇼핑몰 | **계약 협의** | — |

**1-A는 지금 할 수 있다.** 데이터가 이미 있어서 외부 의존이 없다. 다품목 주문의 오차가 즉시 사라진다.

**1-B와 5번이 짝이다.** 우리가 계산을 그만두려면 쇼핑몰이 계산 결과를 줘야 한다. 저장소 간 계약이라 검증 쇼핑몰 일정과 맞물린다.

**4번도 지금 할 수 있다.** 파일명만 바꾸는 정리다.
