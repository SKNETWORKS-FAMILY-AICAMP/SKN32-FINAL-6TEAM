...
- [D-020 여행 시작 설문 — 「일정이 꼬이면」을 사용자가 고른다](D-020-trip-survey-and-ask-first.md) — 2026-09-24 회의. 15번 바로 바꿈/먼저 물어봄, 16번 여유 → 밀도 목표. 결정 15·통지 모델이 사용자별로 갈린다. 세부 선택지는 담당 팀이 정한다.
...
| ID | 무엇 | 상태 |
|---|---|---|
| [D-017](D-017-watch-cadence-golden-time.md) | **감시는 소스별 공유 수집, 주기는 응답 목표와 호출 예산으로 계산한다** (기본 3분) | **초안.** 설정값(재난문자 1,000 · Google 한도)은 먼저 반영됐다. 에어코리아 주기는 팀 논의 필요 |
